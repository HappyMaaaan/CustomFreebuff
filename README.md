...
- **Freebuff runs but no window is visible** — when a program starts another program in the background, Windows often opens its window *behind* everything else (foreground lock). The studio now detects this and brings the window to the front automatically after launching. If it still does not appear, open the **What happened (diagnostics)** panel: it now includes the window's real state (visible or not, position, size), which tells you whether the window exists but is hidden.
