...
export async function freebuffProcessInfo() {
  ...
  return { running, staleCount, total: processes.length }
}
...
/**
 * Removes leftover Freebuff processes that have no window (a previous session
 * that did not fully exit). These are dead weight and can block a new launch
 * via Electron's single-instance lock.
