...
function listWindowsProcesses() {
  return new Promise((resolve) => {
    // windowsHide: true is REQUIRED. tasklist is a console app: without it,
    // every call pops a console window (Windows Terminal as the default
    // console host flashes on screen). The launcher polls /api/state every
    // 2 s, so this ran twice per second and looked like a blinking terminal.
    execFile('tasklist', ['/V', '/FO', 'CSV', '/FI', `IMAGENAME eq ${EXE_NAME}`], { windowsHide: true }, (err, stdout) => {
...
function listOrchestratorProcesses() {
  return new Promise((resolve) => {
    ...
    execFile('wmic', ['process', 'where', "name='bun.exe'", 'get', 'ProcessId,ExecutablePath', '/format:list'], { windowsHide: true }, (err, stdout) => {
      if (err) return resolve([])
      const rows = []
      let cur = {}
      for (const line of stdout.split(/\r?\n/)) {
...
/** Full readable snapshot of every Freebuff-related process, for diagnostics. */
export async function processSnapshot() {
  const rows = process.platform === 'win32' ? await listWindowsProcesses() : await listUnixProcesses()
  const windows = rows.filter((p) => p.hasWindow).map((p) => ({ pid: p.pid, kind: 'window' }))
  const windowless = rows.filter((p) => !p.hasWindow).map((p) => ({ pid: p.pid, kind: 'helper' }))
  const orchestrators = await listOrchestratorProcesses()
  return { windows, windowless, orchestrators, total: windows.length + windowless.length + orchestrators.length }
}

/**
 * Waits until no windowed Freebuff instance is running (the previous session
 * has fully released Electron's single-instance lock), or the timeout elapses.
 * Returns true if the instance exited in time.
 */
export async function waitForInstanceExit(timeoutMs = 8000) {
...
export async function killStaleProcesses() {
  ...
  for (const row of rows) {
    ...
    await new Promise((resolve) => {
      execFile('taskkill', ['/PID', String(row.pid), '/F'], { windowsHide: true }, (err) => {
        if (!err) killed++
        resolve()
      })
    })
  ...
  for (const o of orchestrators) {
    await new Promise((resolve) => {
      execFile('taskkill', ['/PID', String(o.pid), '/F'], { windowsHide: true }, (err) => {
        if (!err) killed++
        resolve()
      })
    })
...
function spawnDetached(exePath, args) {
  // NOTE: windowsHide MUST stay off. On Windows, `windowsHide: true` sets
  // STARTUPINFO.wShowWindow = SW_HIDE, which makes the OS create the app's
  // window HIDDEN (vis=false) — the exact "Freebuff runs but no window" bug.
  const opts = {
    detached: true,
    stdio: 'ignore',
  }
