/**
 * lib/launcher.mjs — Freebuff Desktop discovery and launch.
 *
...
export function isRunning() {
  return new Promise((resolve) => {
    if (process.platform === 'win32') {
      execFile('tasklist', ['/FI', `IMAGENAME eq ${EXE_NAME}`], (err, stdout) => {
        if (err) return resolve(false)
        resolve(/Freebuff\.exe/i.test(stdout))
      })
    } else {
 ...
 * If an instance is already running, it will NOT be relaunched (Electron's
 * single-instance lock) — the caller reports that instead of double-launching.
 */
