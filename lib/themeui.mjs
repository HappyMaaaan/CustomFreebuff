 ...
 * mounts a small "Thèmes" entry point (floating button + panel) in the app
...
export function makeThemeUiSource(uiPort) {
  ...
  var SHADOW_LABELS = { none: 'Aucune', soft: 'L\u00e9g\u00e8re', medium: 'Moyenne', forte: 'Forte' };
  ...
  function applyTheme(id, name) {
    fetch(API + '/api/apply', {
      ...
      .then(function (res) {
        if (res && res.ok) {
          toastMsg('Th\u00e8me \u00ab ' + name + ' \u00bb activ\u00e9 \u2713');
          refresh();
        } else {
          toastMsg((res && res.message) || 'Impossible d\u2019activer le th\u00e8me.');
        }
      })
      .catch(function () { toastMsg('Studio hors ligne \u2014 activation impossible.'); });
  }
  ...
  function restore() {
    fetch(API + '/api/restore', { method: 'POST' })
      ...
      .then(function (res) {
        if (res && res.ok) {
          toastMsg('Look d\u2019origine restaur\u00e9.');
          refresh();
        } else {
          toastMsg((res && res.message) || 'Impossible de restaurer.');
        }
      })
      .catch(function () { toastMsg('Studio hors ligne \u2014 restauration impossible.'); });
  }
  ...
  function renderThemes(themes, activeId) {
    ...
    if (!themes || !themes.length) {
      ...
      empty.textContent = 'Aucun th\u00e8me disponible.';
    ...
    themes.forEach(function (t) {
      ...
      actBtn.textContent = t.id === activeId ? 'Actif \u2713' : 'Activer';
  ...
  function refresh() {
    fetch(API + '/api/state', { cache: 'no-store' })
      ...
      .then(function (s) {
        if (!s || typeof s !== 'object') { setStatus('err', 'Studio hors ligne'); return; }
        activeThemeId = s.mode === 'theme' ? s.themeId : null;
        if (s.debugAlive && s.connected > 0) {
          setStatus('ok', 'Injection active \u2014 ' + s.connected + (s.connected > 1 ? ' fen\u00eatres' : ' fen\u00eatre'));
        } else if (s.debugAlive) {
          setStatus('warn', 'En attente de connexion\u2026');
        } else if (s.running) {
          setStatus('warn', 'Freebuff tourne sans port de debug');
        } else {
          setStatus('warn', 'Freebuff ferm\u00e9');
        }
        renderThemes(s.themes || [], activeThemeId);
      })
      .catch(function () {
        setStatus('err', 'Studio hors ligne \u2014 lancez CustomFreebuff');
      });
  }
  ...
  function renderComponentsList() {
    ...
    COMPONENT_KEYS.forEach(function (key) {
      ...
      summary.textContent = count ? (count + (count > 1 ? ' r\u00e9glages modifi\u00e9s' : ' r\u00e9glage modifi\u00e9')) : 'Par d\u00e9faut';
  ...
  function onComponentChange(prop, value) {
    ...
    editingDirty = true;
    setEditStatus('Aper\u00e7u en direct \u2014 non enregistr\u00e9');
    renderComponentsList();
    schedulePreview();
  }
  ...
  function resetComponent() {
    ...
    renderComponentsList();
    editingDirty = true;
    setEditStatus('Composant r\u00e9initialis\u00e9 \u2014 aper\u00e7u appliqu\u00e9');
    schedulePreview();
    toastMsg('Composant r\u00e9initialis\u00e9.');
  }
  ...
  function openEditor(id) {
    ...
    editName.textContent = (theme.name || theme.id) + (theme.builtin ? '' : '  (personnalis\u00e9)');
    setEditStatus('');
    renderComponentsList();
    showEditMain();
  ...
  function saveEditor() {
    fetch(API + '/api/themes/save', {
      ...
      .then(function (res) {
        if (res && res.ok) {
          toastMsg('Th\u00e8me enregistr\u00e9 et activ\u00e9 \u2713');
          editingDirty = false;
          refresh();
          viewEdit.hidden = true;
          viewList.hidden = false;
        } else {
          toastMsg((res && res.message) || 'Enregistrement impossible.');
        }
      })
      .catch(function () { toastMsg('Studio hors ligne \u2014 enregistrement impossible.'); });
  }
    ...
    fetch(API + '/api/themes/reset', {
      ...
      .then(function (res) {
        if (res && res.ok && res.theme) {
          ...
          renderComponentsList();
          editingDirty = true;
          setEditStatus('Th\u00e8me r\u00e9initialis\u00e9 \u2014 aper\u00e7u appliqu\u00e9');
          schedulePreview();
          toastMsg('Th\u00e8me r\u00e9initialis\u00e9.');
        } else {
          toastMsg((res && res.message) || 'Reset impossible.');
        }
      })
      .catch(function () { toastMsg('Studio hors ligne \u2014 reset impossible.'); });
  }
  ...
  function mount() {
    ...
    sub.textContent = 'Thèmes — CustomFreebuff';
    ...
    restoreBtn.textContent = 'Restaurer le look d\u2019origine';
    restoreBtn.addEventListener('click', restore);
    var note = document.createElement('div');
    note.className = 'fbt-note';
    note.textContent = 'Injection locale uniquement : aucun fichier de Freebuff n\u2019est modifi\u00e9.';
    ...
    backBtn.textContent = '\u2190';
    backBtn.title = 'Retour aux thèmes';
    ...
    editTitle.textContent = '\u00c9diteur de th\u00e8me';
    ...
    hint.textContent = 'Chaque couleur est un token de design : elle pilote plusieurs variables CSS \u00e0 la fois. Les composants peuvent surcharger ces tokens localement \u2014 un bouton peut être personnalisé sans toucher aux inputs.';
    ...
    compBack.title = 'Retour aux composants';
    ...
    compResetBtn.textContent = 'R\u00e9initialiser ce composant';
