...
import { makeThemeUiSource } from './themeui.mjs'
 ...
 * (SPA reloads, thread windows). When `themeUiPort` is set, also injects the
 * Theme Engine panel (VS0) so the app itself offers a "Thèmes" entry point.
 * Returns the connected client.
 */
export async function themeTarget(wsUrl, css, colorScheme, log = () => {}, themeUiPort = null) {
  ...
  if (themeUiPort) {
    const uiSource = makeThemeUiSource(themeUiPort)
    await client.send('Page.addScriptToEvaluateOnNewDocument', { source: uiSource })
    await client.send('Runtime.evaluate', { expression: uiSource, returnByValue: true })
  }
 ...
 */
export function watchAndTheme({ debugPort, css, colorScheme, themeUiPort = null, onStatus, log = () => {} }) {
  let stopped = false
  ...
  async function tick() {
    ...
    try {
      ...
      for (const target of targets) {
        ...
        themeTarget(target.webSocketDebuggerUrl, css, colorScheme, log, themeUiPort)
          .then((client) => {
            clients.add(client)
            client.onEvent = (msg) => {
              // An app reload does not restart our session; the CSS (and the
              // Theme Engine panel) are re-applied by
              // addScriptToEvaluateOnNewDocument. We re-assert them anyway
              // after each document load / navigation.
              if (
                ...
                if (themeUiPort) {
                  client
                    .send('Runtime.evaluate', {
                      expression: makeThemeUiSource(themeUiPort),
                      returnByValue: true,
                    })
                    .catch(() => {})
                }
