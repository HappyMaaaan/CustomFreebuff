 ...
 *  Freebuff DOM: 1000+ real <button>s, the chat box is a <textarea>).
 *  Chat messages (.bubble/.msg) are deliberately NOT themable: they are a
 *  special element of the app that must keep its own look (a "card" here
 *  means an actual .card panel, not every chat bubble). */
...
/** The app's surfaces the global shape/effects/transition apply to (same
 *  family as the component selectors, so per-component overrides win by
 *  cascade). Chat messages (.bubble/.msg) are excluded on purpose: a chat
 *  holds dozens of them, so per-message shadows, glass or transitions would
 *  make the interface unreadable and are GPU-heavy. Messages only keep the
 *  motion "enter" animation (see VS6), never the rest. */
export const SHAPE_SELECTORS =
  'button,input,textarea,select,.card,aside,.sidebar,.modal,[role="dialog"]'

/** The interactive controls that get the per-state TRANSFORMS (VS6/VS7).
 *  Containers (sidebars, modals, cards, panels) never scale or move on
 *  hover — only clickable controls express the motion personality, so the
 *  rest of the app stays calm. The transition still applies to every
 *  surface (SHAPE_SELECTORS); only the transform rules are restricted. */
export const MOTION_STATE_SELECTORS = 'button,input,textarea,select'
...
/** Appends a state suffix to every selector of `base`, keeping each scoped. */
function surfaceStateSelector(suffix, base = SHAPE_SELECTORS) {
  return base.split(',')
...
export function themeToCss(theme) {
  ...
  if (motionActive(motion)) {
    ...
    // The transforms stay on the interactive controls only — chat messages
    // and containers never grow or move on hover.
    if (motion.hover.translateY !== 0 || motion.hover.scale !== 1) {
      css += `\n${surfaceStateSelector(':hover', MOTION_STATE_SELECTORS)}{transform: ${motionTransform(motion.hover)} !important}`
    }
    if (motion.active.scale !== 1) {
      css += `\n${surfaceStateSelector(':active', MOTION_STATE_SELECTORS)}{transform: ${motionTransform(motion.active)} !important}`
    }
    if (motion.focus.scale !== 1) {
      css += `\n${surfaceStateSelector(':focus, :focus-visible', MOTION_STATE_SELECTORS)}{transform: ${motionTransform(motion.focus)} !important}`
    }
    if (motion.enter) {
...
/*                                                                     */
/* A theme is portable: the whole model (tokens, components, states,   */
/* shape, effects, motion, custom CSS) serializes to a single JSON     */
/* text with a versioned envelope — the .freebuff file. Import         */
/* validates the envelope (format + version + compatibility) before    */
/* installing, and normalizeTheme sanitizes whatever survives.         */
/* ------------------------------------------------------------------ */

export const THEME_FILE_FORMAT = 'customfreebuff-theme'
export const THEME_FILE_VERSION = 1

/** "My Theme!" → "my-theme" — used for file names and imported ids. */
...
/** Serializes a theme into the portable .freebuff JSON text. The theme is
 *  normalized first, so an export is always clean. */
  ...
  return JSON.stringify(
    {
      ...
      name: t.name,
      // Reserved for future binary assets (fonts, images…); themes are
      // pure data today, so this envelope stays minimal.
      theme: t,
    },
    null,
    2
...
/** Parses + validates a .freebuff file (or a bare theme JSON object).
 *  Returns { ok, theme?, name?, code?, error? } — code is a stable machine
 *  key ('invalid-json' | 'wrong-format' | 'invalid-version' |
 *  'newer-version'), error a human message for the editor. */
export function parseThemeFile(text) {
  ...
  if (!data || typeof data !== 'object' || Array.isArray(data)) {
    return fail('wrong-format', 'The file does not contain a theme.')
  }
  const version = data.version === undefined ? 1 : Number(data.version)
  if (!Number.isFinite(version) || version < 1) {
    return fail('invalid-version', 'The theme file version is invalid.')
  }
  if (version > THEME_FILE_VERSION) {
    return fail('newer-version', 'This theme was created with a newer version of CustomFreebuff and cannot be imported here.')
  }
  let raw = data
  if (data.format === THEME_FILE_FORMAT) {
    raw = data.theme
    if (!raw || typeof raw !== 'object' || Array.isArray(raw)) {
      return fail('wrong-format', 'The theme file is missing its theme payload.')
    }
  } else if (data.format !== undefined) {
    return fail('wrong-format', 'This is not a CustomFreebuff theme file.')
  } else if (!raw.tokens || typeof raw.tokens !== 'object' || Array.isArray(raw.tokens)) {
    // No format field: accept a bare theme object (raw theme JSON shared
    // before .freebuff existed) — but it must look like a theme.
    return fail('wrong-format', 'This is not a CustomFreebuff theme file (no theme tokens found).')
  }
  const theme = normalizeTheme(raw)
  const name = String(data.name || theme.name || theme.id || 'Imported theme').trim() || 'Imported theme'
